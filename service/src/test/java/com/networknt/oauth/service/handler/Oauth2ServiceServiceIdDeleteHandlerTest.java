...
public class Oauth2ServiceServiceIdDeleteHandlerTest {
    ...
    public void testOauth2ServiceServiceIdDeleteHandler() throws ClientException, ApiException {
        ...
        HttpDelete httpDelete = new HttpDelete("http://localhost:8080/v1/oauth2/service/serviceId");
        /*
        Client.getInstance().addAuthorization(httpPost);
        try {
            CloseableHttpResponse response = client.execute(httpDelete);
            Assert.assertEquals(200, response.getStatusLine().getStatusCode());
            Assert.assertEquals("deleteService", IOUtils.toString(response.getEntity().getContent(), "utf8"));
        } catch (Exception e) {
            e.printStackTrace();
        }
        */
